...
const CommonCard = ({ icon, title, desc, footerContent }: any) => {
  return (
    <Card className='flex bg-gray-100 flex-col gap-6 rounded-2xl p-8   transition duration-300 hover:bg-white hover:shadow-2xl hover:shadow-gray-600/10   cursor-pointer '>
      <CardHeader className='p-0'>
        ...
        {title ? (
          <CardTitle className='text-xl capitalize max-w-[250px] text-ellipsis overflow-hidden whitespace-nowrap font-semibold text-gray-950 '>
            {title}
          </CardTitle>
        ) : null}
